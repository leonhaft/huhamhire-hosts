...
SF_mirror_list = {'superb-dca2': ('Superb Internet', 'United States'), 'aarnet': ('AARNet', 'Melbourne Australia'), 'cdnetworks-kr-1': ('CDNetworks', 'Seoul, Korea, Republic of'),
                  ...
                  'internode': ('Internode', 'Adelaide, Australia'), 'iweb': ('iWeb', 'Montreal, QC'), 'jaist': ('Japan Advanced Institute of Science and Technology', 'Nomi, Japan'),
                  'kent': ('University of Kent', 'Bristol, United Kingdom'), 'nchc': ('National Center for High-Performance Computing', 'Taipei, Taiwan'),
                  'netcologne': ('NetCologne', 'Köln, Germany'), 'softlayer': ('Softlayer', 'Dallas, TX'), 'switch': ('SWITCH', 'Zurich, Switzerland'),
                  'tenet': ('TENET', 'Wynberg, South Africa'), 'ufpr': ('Centro de Computacao Cientifica e Software Livre', 'Curitiba, Brazil'), 'voxel': ('Voxel Hosting', 'New York, NY'),
                  'waix': ('Waix', 'Perth, Australia')}  #Sourceforge 镜像站点信息：{'Short Name': ('Provider', 'Location')}
...
def help():
    type = sys.getfilesystemencoding()
    print ('用法: python hosts_setup_OnL.py [-SFM] [-SF (server_shotname)] [-H] \n\n').decode('utf-8').encode(type)
    print ('选项: \n').decode('utf-8').encode(type)
    print ('    -SFM                     设定使用 http://www.mirrorservice.org/ 上的 hosts 文件').decode('utf-8').encode(type)
    print ('    -SF  (server_shotname)   设定使用 SourceForge 镜像服务器上的 hosts 文件').decode('utf-8').encode(type)
